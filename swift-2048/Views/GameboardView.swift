...
//    }
  ...
  /// 与えられた位置が有効かどうかを返す。Used for bounds checking.
  ...
  /// 与えられた位置にタイルを追加することでゲームボードを更新する。そのタイルは'ポップ'アニメーションで追加される。
  ...
  /// 1つのタイルがある位置から他の位置に移動したときにゲームボードを更新する。
  /// その移動で2つのタイルがぶつかって1つの新しいタイルになる場合、その新しい位置に移動した後にタイルが'ポップ'する。
  func moveOneTile(from: (Int, Int), to: (Int, Int), value: Int) {
    ...
    /// 2つのタイルがそれぞれの位置から共通の位置に移動したときにゲームボードを更新する。
    /// このアクションは常にタイルの衝突を表現して、両方のタイルが移動した後に合体したタイルが'ポップ'する。
